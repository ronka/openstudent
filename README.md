...
8. הרץ npm run lint ו-npx tsc --noEmit. דווח בבירור על כל כשל שנותר ואל תסתיר אותו.
...
לפני פתיחת Pull Request, ודאו ששתי הפקודות מסתיימות בהצלחה.
...
מצאתם באג, רעיון לשיפור או פיצ'ר שחסר? אפשר לפתוח Issue או Pull Request. הוראות העבודה נמצאות ב־[CONTRIBUTING.md](./CONTRIBUTING.md). אני אעבור על Pull Requests ואטפל במיזוג ובהפצה. אין צורך לפרסם builds, לבצע deployment או להגדיר חשבון EAS כדי לתרום לפרויקט.
